...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        # TODO: Add image names to the instance_prompt
        # 法1：形式：<class> in style_<num>
        self.instance_prompt = [f"{image.stem} in {instance_prompt}" for image in self.instance_images_path]
        # # 法2：形式：a sks <style>
        # self.instance_prompt = [f"a sks {image.stem}" for image in self.instance_images_path]
        ...
        if class_data_root is not None:
            ...
            # # TODO:Add image names to the class_prompt
            # self.class_prompt = [f"{image.stem}: {class_prompt}" for image in self.class_images_path]
        else:
...
def main(args):
   ...
   #unet.requires_grad_(False)
    ...
    # vae_lora_config = unet_lora_config
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            # model_input.shape = (bsz, channels, height, width) = (1, 3, 512, 512)
            model_input = vae.encode(pixel_values).latent_dist.sample()

            # here, model_input.shape changed to (1, 4, 64, 64)
            ...
            # TODO: Reve
            # batch["input_ids"]:编码自风格名（如“style_00”)
            encoder_hidden_states = encode_prompt(
                text_encoder,
                batch["input_ids"], # input (string -> array)
