...
def parse_args(input_args=None):
    ...
    # TODO: add lora for text_encoder
    parser.add_argument(
        "--train_text_encoder",
        action="store_true",
...
def main(args):
   ...
   #unet.requires_grad_(False)
    ...
    unet_lora_config = LoraConfig(
        ...
        # target_modules='.*?to_k.*?|.*?to_q.*?|.*?to_v.*?|.*?to_out\\.0.*?|.*?resnets.*?conv.*?|time_embedding\\.linear.*|.*?add_k_proj.*?|.*?add_v_proj.*?|.*?add_out_proj.*?',
    )
    unet.add_adapter(unet_lora_config)

    # TODO: add lora to text_encoder
